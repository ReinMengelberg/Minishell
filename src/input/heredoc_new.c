...
/*   Updated: 2025/06/28 16:16:05 by rbagin        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int handle_heredoc(char *delimiter, t_shell *shell)
{
    ...
    if (pid == 0) {
        ...
        // We don't reach here - collect_heredoc_input always exits
    }
    ...
    if (WIFSIGNALED(status)) {
        close(pipe_fd[0]);
        g_signal_received = WTERMSIG(status);  // Propagate signal to parent
        shell->exit_status = 130;  // Standard for SIGINT
        return (-1);
    }
